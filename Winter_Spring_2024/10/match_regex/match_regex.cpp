...
void view_directory(const std::filesystem::path &path, const std::string &regex_str) {
    if (std::filesystem::exists(path) && std::filesystem::is_directory(path)) {
        std::regex regex_filter(regex_str);
        for (const auto &entry : std::filesystem::recursive_directory_iterator(path)) {
            if (std::regex_match(entry.path().filename().string(), regex_filter)) {
