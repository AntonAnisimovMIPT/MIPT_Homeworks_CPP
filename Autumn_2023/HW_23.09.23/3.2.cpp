#include <iostream>
#include <cmath> // только для подсчета корня!!! (я мог пойти по сложному пути, не используя формулу герона, выражая углы по теореме косинусов, но это бы усложнило бы код, но суть реализации класса не изменило бы)
